...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo Member Baru..👋 @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Sedang di Prosess, Mohon Bersabar ⌛',
				success: '✔️ Succes ✔️',
                                levelon: '❬ ✔ ❭ *Sugoi Dekai* level diaktifkan*',
				leveloff: ' ❬ X ❭  *Gomenasai* level di nonaktifkan*',
				levelnoton: '❬ X ❭ *Gomenasai* Level Tidak Aktif*',
				levelnol: '*Gomenasai* Level Kamu Masih* 0 °-°',
				error: {
					stick: '[❗] *Gomenasai*, terjadi kesalahan saat pengubahan gambar ke sticker ❌',
				...
				only: {
					...
					Badmin: '[❗] *Jadikan Bot Sebagai Admin Desu* ❌',
...
case 'timer':
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += ` *Wakataa..* Menurunkan Jabatan\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`*Wakataa..* Menurunkan Jabatan @${mentioned[0].split('@')[0]}\n 𝐣𝐚𝐝𝐢 𝐚𝐝𝐦𝐢𝐧 𝐠𝐫𝐨𝐮𝐩 _*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '*Wakataa..* Menaikkan Jabatan Desu :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`*Wakataa..* Menaikkan Jabatan Desu : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
      options = {
          text: `「 *SELF WHATSAPP* 」\n\n_Request by_ : *@${sender.split("@s.whatsapp.net")[0]}\n\nLink WhatsApp Onichan : *https://wa.me/${sender.split("@s.whatsapp.net")[0]}*\n*Or ( / )*\n*https://api.whatsapp.com/send?phone=${sender.split("@")[0]}*`,
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Owner", vcard: vcard}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Okyakusama',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'BAKAJANAINO? Usernya Gak ada?', text, {quoted: mek})
                                                if (!isUser) return reply(mess.only.daftarB)
						if (!isGroup) return reply(mess.only.group)
						...
						reply('Gomenasai User Tidak Valid ')
					}
					break
				case 'snow':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Kirim perintah ${prefix}snowwrite teks1|teks2, contoh ${prefix}snowwrite bot|levi`)
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Kirim perintah ${prefix}marvellogo teks, contoh ${prefix}marvellogo bot bot`)
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.wait)
		...
		case 'spamcall':
		 ...
		 if (!isGroup) return reply(mess.only.group)
		   ...
		   if (!isGroup) return reply(mess.only.group)
                   ...
                   break
				case 'wiki':
					if (args.length < 1) return reply('Bakajanaino? Mau Cari apa?')
					tels = body.slice(6)	
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Title* : ${anu.title}\n\n*Chottomatte Onichan Video Sedang Dikirim..Jangan Dispam DESU..!*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
				...
				case 'testime':
					setTimeout( () => {
					client.sendMessage(from, 'Gomenasai Waktu Berakhir', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					...
					break
				case 'semoji':
					if (args.length < 1) return reply('Hadeh Onichan Emojinya mana ?')
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('Hadeh Teksnya mana ?')
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
		...
		if (!isGroup) return reply(mess.only.group)
               ...
               if (data.error) return reply(data.error)
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Daftar Yang Telah Terblockir :\n'
				...
				case 'neonime':
                  ...
                  if (args.length < 1) return reply(`Masukan Teks\nContoh : ${prefix}bot bot`)
                data = await getBuffer(`https://docs-jojo.herokuapp.com/api/blackpink?text=${body.slice(7)}`)
                client.sendMessage(from, data, image, {quoted: mek, caption: body.slice(7)})
                break
				case 'tts':
				   client.updatePresence(from, Presence.recording) 
				   if (args.length < 1) return client.sendMessage(from, 'Hadeh..Kode Bahasanya mana?', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Bakaa...Teksnya Berlebihan')
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Daftar Admin Group *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'meme':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Onegaishimasu'})
					break
				case 'memeindo':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'block':
					...
					client.sendMessage(from, `Wakataa Onichan, memblokir ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					if (!isGroup) return reply(mess.only.group)
					client.updatePresence(from, Presence.composing) 
				...
				case 'setppbot':
				client.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Onichan Kirim Gambar Dengan Hastag ${prefix}setbotpp atau tag gambar yang sudah dikirim`)
					if (!isOwner) return reply(mess.only.ownerB)
					enmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(enmedia)
					await client.updateProfilePicture(botNumber, media)
					reply('Onichan Arigatou😗')
					break
				case 'bc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
		    ...
		    if (!isGroup) return reply(mess.only.group)
                    ...
                    hasil = await getBuffer(data.result)
                    client.sendMessage(from, hasil, image, {quoted: mek, caption: 'Onegaishimasu'})
                    break
                     case 'leave':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isGroupAdmins) return reply(mess.only.admin)
				...
				case 'chord':
					if (args.length < 1) return reply('Judul Lagunya Onichan?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Lirik lagunya mana ?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
	   ...
	   if (!isGroup) return reply(mess.only.group)
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Yang mau di add sapa ?')
					if (args[0].startsWith('08')) return reply('Gunakan kode negara nya ')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Gomenasai Onichan Gagal Menambahkan..Mungkin Nomor Diprivate')
					}
					break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Ja Mattane Onichan 😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return client.sendMessage(from, "Stres Onichan? ", text, { quoted: mek })
		           if (stdout) {
		...
		if (!isGroup) return reply(mess.only.group)
				...
				case 'ocr':
                    ...
                    reply('Arigatou😗 Atas Laporannya .')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
		if (!isGroup) return reply(mess.only.group)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Gomenasai Gagal, pada saat pengubahan ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Gomenasai Gagal Onichan ❌')
			...
			if (!isGroup) return reply(mess.only.group)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Gomenasai Error, pada saat pengubahan video ke mp3 ❌')
						buffer = fs.readFileSync(ran)
		  ...
		  if (!isGroup) return reply(mess.only.group)
		   ...
		   if (!isGroup) return reply(mess.only.group)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf bot|bot`)
					reply(mess.wait)
					...
					if (Number(args[0]) === 1) {
						...
						reply('ketik 1 untuk mengaktifkan, nonaktif untuk menonaktifkan fitur')
					}
					break	
				case 'bucin':
            ...
            case 'leveling':
                ...
                if (args[0] === 'aktif') {
                    ...
                    reply(' *Ketik perintah aktif untuk mengaktifkan, nonaktif untuk menonaktifkan* \n *Contoh: ${prefix}leveling aktif*')
                }
            break
                                case 'infogempa':
					...
					if (!isGroup) return reply(mess.only.group)
                                ...
                                /*case 'jsholat':
                                        tels = body.slice(8)
                                        if (args.length < 1) return reply('Daerahnya dimana ?gk punya rumah lu?')
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('ALASAN AFKNYA APAAN?')
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        var nom = mek.participant
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *SEDANG AFK ${tels} JANGAN GANGGU*`,
					...
					if (!isGroup) return reply(mess.only.group)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Atteru Onichan ...'})
                                        break
                                case '8bit':
                                        var gh = body.slice(5)
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana \nContoh: ${prefix}8bit bot|bot')
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=bit8&text1=${teks1}&text2&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Onegashimasu...'})
                                        break
                                case 'randomkpop':
				...
				case 'clone':
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana \nContoh: ${prefix}pubglogo bot|random')
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
